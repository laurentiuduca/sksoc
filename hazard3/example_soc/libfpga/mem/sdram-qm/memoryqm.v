...
always@(posedge clk or negedge rst_x) begin
    ...
    reg [15:0] rerr=0;
    assign err=rerr;
    always@(posedge clk_sdram or negedge rst_n_sdram) begin
	...
	8'd20: begin
		if(!w_busy) begin
			//define it at the beggining of always
			`ifdef SDRAM_RAW
			// read what we've wrote
			r_rd <= 1;
			ramstate <= 8'd21; 
			`else
			ramstate <= 0;
	...
	`ifdef SDRAM_RAW
        ...
        8'd22: begin 
                if(!w_busy) begin
			if(((w_dram_odata[7:0]   != r_mwdata[7:0]) && r_mmask[0]) ||
			   ((w_dram_odata[15:8]  != r_mwdata[15:8]) && r_mmask[1]) ||
			   ((w_dram_odata[23:16] != r_mwdata[23:16]) && r_mmask[2]) ||
			   ((w_dram_odata[31:24] != r_mwdata[31:24]) && r_mmask[3])
				) begin
				// try to write again
				if(rerr < 16'hffff)
					rerr <= rerr + 1;
				...
				ramstate <= 1;
				`ifdef SIM_MODE
				$display("w_dram_odata=%x r_mmask=%x r_mwdata=%x", w_dram_odata, r_mmask, r_mwdata);
				$finish;
				`endif
			end else
				ramstate <= 0;
...
`ifdef SIM_MODE
    m_sdram_sim #(`MEM_SIZE/2) idbmem(.CLK(clk_sdram), .w_addr(r_maddr), .w_odata(w_dram_odata),
