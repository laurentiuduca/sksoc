...
module hazard3_ethernet #(
    ...
    reg [15:0]
        txsize = 0, rxsize = 0, rxcnt = 0, ndiscarded = 0, rxdiscard = 0, rxread = 0, rxwrote = 0;
...
`endif
    ...
    task rxoctet(input int b);
        ...
        if (rxdiscard) begin
            if (rxdiscard == 1) ndiscarded = ndiscarded + 1;
...
`ifndef rxrealread
            prdata <= {
                brrx.m[rxmaddrb+3], brrx.m[rxmaddrb+2], brrx.m[rxmaddrb+1], brrx.m[rxmaddrb+0]
            };
            ctrlstate <= 6;
`else
...
`endif
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (ctrlstate == `CTRLSTATE_SENDPACKET) txstate <= 1;
...
`endif
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (rxcnt2 < 1000) rxcnt2 <= rxcnt2 + 1;
            else begin
                rxcnt2 <= 0;
                ret = checkrx();
            end
