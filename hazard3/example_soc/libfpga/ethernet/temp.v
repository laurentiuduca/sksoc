    ...
    always @(posedge tx_clk or negedge rst) begin
        ...
        if (txstate == 10) begin
            ...
            if (tx_axis_tready) begin
                $display("tx_axis_tready cnt=%d tx_frame[i]=%x mac_tx_state_reg=%d", cnt,
                         tx_frame[cnt], mac_tx_state_reg);
                tx_axis_tdata <= tx_frame[cnt];
                tx_axis_tvalid <= 1'b1;
...
`endif
    ...
    task phy_replay;
        ...
        begin
            ...
            for (j = 0; j < phy_len; j = j + 1) begin
                ...
                $display("rx state_reg=%d rxd=%x phy_buf[3:0]=%x",
                         dut.eth_mac_1g_inst.axis_gmii_rx_inst.state_reg,
                         dut.eth_mac_1g_inst.axis_gmii_rx_inst.gmii_rxd_d4, phy_buf[j][3:0]);
                // high nibble
                @(posedge rx_clk);
                drive_rx_nibble(phy_buf[j][7:4]);
                $display("rx state_reg=%d rxd=%x phy_buf[7:4]=%x",
                         dut.eth_mac_1g_inst.axis_gmii_rx_inst.state_reg,
    ...
    task compare_frames;
        ...
        begin
            mismatch = 0;
            $display("TB: comparing first %0d bytes (tx_len=%0d, rx_len=%0d)", tx_len, tx_len,
                     rx_len);
            for (k = 0; k < tx_len && k < rx_len; k = k + 1) begin
                if (rx_buf[k] !== tx_frame[k]) begin
                    $display("Mismatch at byte %0d: tx=%02x rx=%02x phy_buf=%02x", k, tx_frame[k],
                             rx_buf[k], phy_buf[k+7]);
                    mismatch = mismatch + 1;
                end
            end
            if (mismatch == 0 && rx_len == tx_len) begin
                ...
                $display("TEST FAILED: mismatches=%0d tx_len=%0d rx_len=%0d", mismatch, tx_len,
                         rx_len);
            end
            // finish simulation
            #1000;
