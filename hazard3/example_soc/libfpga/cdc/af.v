...
module asyncfifo #(
    ...
    always @(posedge wclk or negedge i_wrst_x) begin
        if (!i_wrst_x) begin
            raddr_gray1 <= 0;
            raddr_gray2 <= 0;
        end else begin
    ...
    always @(posedge rclk or negedge i_rrst_x) begin
        if (!i_rrst_x) begin
            waddr_gray1 <= 0;
            waddr_gray2 <= 0;
        end else begin
...
`ifdef dbgfifo
            if (rcnt < 1) begin
                $display("fifo %1x read: raddr=%x data={%x,%x,%x,%x,%x}", qid, raddr, data[69:38],
                         data[37:34], data[33], data[32], data[31:0]);
                rcnt <= rcnt + 1;
            end
`endif
            raddr <= raddr + 1;
