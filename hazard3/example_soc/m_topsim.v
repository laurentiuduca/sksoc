...
mmcmclock mmcmlaur(
    ...
    reset_sync rstsdram(.clk(clk_sdram), .rst_n_in(locked), .rst_n_out(rst_n_sdram));
`endif
`endif
`ifdef WUKONGDDR3
    ...
    `endif
    wire rst_n_ddram;
    reset_sync rstddram(.clk(i_controller_clk), .rst_n_in(locked), .rst_n_out(rst_n_ddram));    
`endif

    wire rst_n_syst;
    reset_sync rstsyst (.clk(pll_clk),   .rst_n_in(locked), .rst_n_out(rst_n_syst));
